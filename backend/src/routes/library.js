 ...
 */
router.post('/upload', uploadLimiter, upload.single('file'), validateUploadedFile, async (req, res) => {
  try {
    if (!req.file) {
...
router.get('/:id', (req, res) => {
  try {
    ...
    logger.error('Get book error', { userId: req.user.id, bookId: req.params.id, error: err.message });
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    const db = getDb();
    const book = db.prepare('SELECT file_url FROM books WHERE id = ? AND user_id = ?').get(id, req.user.id);
    ...
    if (existsSync(fullPath)) {
      try {
        ...
        logger.warn('Could not delete book file', { userId: req.user.id, bookId: id, error: fileErr.message });
      }
    }
    ...
    logger.error('Delete book error', { userId: req.user.id, bookId: req.params.id, error: err.message });
