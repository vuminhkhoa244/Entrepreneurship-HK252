...
function StatCard({ icon, label, value }: { icon: string; label: string; value: string }) {
  ...
  return (
    <View style={[styles.statCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
      <Ionicons
        name={`${icon}-outline` as ComponentProps<typeof Ionicons>['name']}
        size={24}
        color={colors.accent}
      />
      ...
      {stats && (
        <View style={styles.grid}>
          ...
          <StatCard
            ...
            label="Completed"
            value={stats.completed_books?.toString() || '0'}
          />
          <StatCard
            icon="layers"
            label="Pages Read"
            value={stats.total_pages_read?.toString() || '0'}
          />
          <StatCard
            icon="time"
            label="Reading Time"
            value={formatTime(stats.total_reading_seconds || 0)}
          />
        </View>
      )}
